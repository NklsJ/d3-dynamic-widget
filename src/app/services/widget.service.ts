...
export class WidgetService {
    ...
    public getWidgets( startDate: string, endDate: string ): Observable<Widget[]> {
        const MOCK_DATA = EVENTS;
        const events: Event[] = [];
        const timespanStart = moment(startDate, 'YYYY-MM-DD');
        const timespanEnd = moment(endDate, 'YYYY-MM-DD');
